...
source "azure-arm" "ubuntu20-arm" {
  ...
  managed_image_name                = "consul-terraform-sync-ubuntu-${formatdate("YYYYMMDDhhmm", timestamp())}"
...
build {
  ...
  provisioner "shell" {
    inline       = ["if test -n \"${var.consul_terraform_sync_download_url}\"; then", "/tmp/terraform-consul-azure-daff/examples/scripts/install-consul-terraform-sync --download-url ${var.consul_terraform_sync_download_url};", "else", "/tmp/terraform-consul-azure-daff/examples/scripts/install-consul-terraform-sync --version ${var.consul_terraform_sync_version};", "fi"]
    pause_before = "30s"
  }
}